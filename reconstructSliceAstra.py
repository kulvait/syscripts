...
if ARG.verbose:
	print(
	    "Object has dimensions %dx%d and dtype=%s with min=%f, max=%f, mean=%f."
	    % (sin.shape[0], sin.shape[1], sin.dtype, sin.min(), sin.max(),
	       sin.mean()))
...
def createProjectorConfig(projectorName,
                          projectorID,
                          projectionsID,
                          volumeID,
                          usegpu=True):
...
if ARG.verbose:
	print(
	    "Transformed sin of dimensions %dx%d and dtype=%s with min=%f, max=%f, mean=%f."
	    % (sin.shape[0], sin.shape[1], sin.dtype, sin.min(), sin.max(),
	       sin.mean()))
...
print("Creating projector with det_width=%f and det_count=%f for %d angles." %
      (det_width, det_count, len(angles)))
...
vol_geom = astra.create_vol_geom(row_count, col_count, min_x, max_x, min_y,
                                 max_y)
...
if ARG.verbose:
	print("Output volume has dimensions %dx%d and type %s" %
	      (volume.shape[0], volume.shape[1], volume.dtype))
