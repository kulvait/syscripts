...
parser.add_argument("--gamma", type=str, help="Alpha mask for l1 fitting.")
parser.add_argument("--gamma-ext", type=str, help="EXT to which actual fit shall be performed.")
...
#Copy relevant arrays
		...
		if weight_tv > 0 or weight_cns_tv > 0:
			self.gamma_x_mask = tf.constant(self.gamma[:,1:] & self.gamma[:,:-1], dtype=tf.bool)
			self.gamma_y_mask = tf.constant(self.gamma[1:,:] & self.gamma[:-1,:], dtype=tf.bool)
		if weight_tv > 0:
			self.reduce_k = 5
	...
	def updateEXT(self, ext_prev):
		self.ext_prev = ext_prev
		self.tf_ext_prev_gamma = tf.constant(self.ext_prev[self.gamma], dtype=self.dtype)
	...
	def getTVCnsTensor(self, EXPEXTPREV, B, FF):
		...
		expext = tf.math.divide_no_nan(FF, B)
		dif_tv = tf.math.subtract(EXPEXTPREV, expext)
	...
	def getMinimizer(self):
		def minimizer(x):
			...
			if self.weight_cns_tv > 0:
				...
				#Try this
				ext_prev_tf = tf.constant(np.exp(self.ext_prev), dtype=self.dtype)
				ret += self.getTVCnsTensor(ext_prev_tf, b_tf, ff)
			return ret
		return minimizer
...
with tf.device('/cpu:0'):
	...
	AGM = AlphaGammaConsecutiveMinimizer(b, scaledBasisVec, ff_first, b_avg, initScale=None, alpha=alpha, gamma=gamma, weight_l1=0, weight_tv=0, weight_neg=0, weight_cns_l1=1, weight_cns_tv=1, dtype=tf.float32)
	...
	for k in np.arange(frameCount):
		...
		ext_prev = DEN.getFrame(ARG.gamma_ext, k)
		AGM.updateB(b)
		AGM.updateEXT(ext_prev)
		minimizer = AGM.getMinimizer()
		converged=True
		if ARG.lbfgs:
