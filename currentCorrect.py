...
parser.add_argument("--type", default="IMG", const="IMG", nargs="?", choices=["IMG", "REF"], help="Process just one category of IMG ... actual images, REF ... flat fields.")
parser.add_argument("--target-current-value", type=float, help="Current to correct to [default 100mA]", default=100)
parser.add_argument("--saveinfo", type=str, help="Create info object from those data and save it as...", default=None)
parser.add_argument("--savefig", type=str, help="Create figure from those data and save it as...", default=None)
...
def currentCorrect(ARG):
	...
	if not os.path.exists(ARG.inputDen):
		...
		if len(inf["dimspec"]) != 3:
			raise ValueError("File %s has %d dimensions, shall be 3"%(ARG.inputDen, len(inf["dimspec"])))
		if inf["dimspec"][2] != imageCount:
			raise ValueError("File %s contains %d images but the df contains %d descriptions"%(ARG.inputDen, inf["dimspec"][2], imageCount))
		dimx = inf["dimspec"][0]
		dimy = inf["dimspec"][1]
	DEN.writeEmptyDEN(denFile, [dimx, dimy, imageCount], elementtype=np.dtype("<f4"), force=True)
	...
	for i in range(imageCount):
		...
		DEN.writeFrame(denFile, i, np.array(factor*img, dtype=np.float32), force=True)
