...
parser.add_argument(
    ...
    type=str,
    help=
    "Data of the lowest admissible value after dark field correction, data less or equal to max(dark-frame-inf, 1.67 MAD_frame) will be set to  max(dark-frame-inf, 1.67 MAD_frame).",
    default=None)
...
def writeDenFile(inputTifFiles,
	...
	if darkFrame is not None:
		...
		print("Correction value of the minimum signal is %f" %
		      (minimumAdmissibleValue))
...
#			plt.show()
		...
		if exportInfo:
			info[0, i] = np.float64(scanData[
			    scanData["image_file"].str.contains(fileName)].index[0])
			info[1, i] = np.float64(scanData[
			    scanData["image_file"].str.contains(fileName)]["s_rot"].iloc[0])
			info[2,
			     i] = np.float64(scanData[scanData["image_file"].str.contains(
		...
		if targetCurrentValue is not None:
			#ARG = parser.parse_args([])
			frameCurrent = scanData[scanData["image_file"].str.contains(
			    fileName)]["current"].iloc[0]
	...
	if beamCurrentData is not None:
		axis[0, 1].scatter(beamCurrentData.index,
...
writeDenFile(ARG.inputTifFiles,
             ...
             scanData=scanData,
             beamCurrentData=beamCurrentData,
