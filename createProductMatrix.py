...
parser.add_argument("--inverse",
                    default=None,
                    type=str,
                    help="Store also inverse of the matrix")
parser.add_argument(
    "--count-left",
    default=None,
    type=int,
    help="Number of projections from A to consider equals number of rows.")
parser.add_argument(
    "--count-right",
    default=None,
    type=int,
    help="Number of projections from B to consider equals number of columns.")
...
parser.add_argument(
    ...
    type=int,
    help=
    "Number of threads to use. [defaults to -1 which is mp.cpu_count(), 0 without threading]"
)
...
if infA["dimcount"] != 3 or infB["dimcount"] != 3:
	raise ValueError(
	    "infA[\"dimcount\"] = %d and  infB[\"dimcount\"] =%d but it shall be 3",
	    infA["dimcount"], infB["dimcount"])
...
if ARG.count_left is not None:
	if ARG.count_left <= rowCount:
		...
		raise IOError(
		    "ARG.count_left=%d is higher than number of frames in %s=%d" %
		    (ARG.count_left, ARG.inputFileA, infA["dimspec"][2]))
...
if ARG.count_right is not None:
	if ARG.count_right <= colCount:
		...
		raise IOError(
		    "ARG.count_right=%d is higher than number of frames in %s=%d" %
		    (ARG.count_right, ARG.inputFileB, infB["dimspec"][2]))

if ARG.inverse is not None and colCount != rowCount:
	raise IOError(
	    "To compute inverse rowcount=%d shall be equal to colCount=%d." %
	    (rowCount, colCount))
...
if ARG.threads == -1:
	...
	if ARG.verbose:
		print(
		    "Will be computing products of A=%s and B=%s data with ARG.threads=mp.cpu_count()=%d."
		    % (ARG.inputFileA, ARG.inputFileB, ARG.threads))
