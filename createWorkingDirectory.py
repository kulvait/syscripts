...
for d in subDirs:
	...
	if "processed" in params:
		logfile = glob.glob(os.path.join(params["processed"], '**/reco.log'),
		                    recursive=True)
		if len(logfile) == 0 and ARG.processed_only:
			...
			logcontent = dict(
			    map(str.strip, l.split(':', 1)) for l in logfile.readlines())
			if "rotation_axis_offset_reco" in logcontent:
				params["rotation_axis_offset_reco"] = logcontent[
				    "rotation_axis_offset_reco"]
			if "effective_pixel_size" in logcontent:
				params["effective_pixel_size"] = logcontent[
				    "effective_pixel_size"].rsplit(" ")[0]
			if "sample_detector_distance" in logcontent:
				params["sample_detector_distance"] = logcontent[
				    "sample_detector_distance"].rsplit(" ")[0]
	else:
	...
	params["pixel_size_magnification"] = h5["entry/hardware/%s/magnification" %
	                                        cam][0]
	params["pixel_size_x"] = float(params["pixel_size_camera"]) / float(
	    params["pixel_size_magnification"])
	params["pixel_size_y"] = float(params["pixel_size_camera"]) / float(
	    params["pixel_size_magnification"])
