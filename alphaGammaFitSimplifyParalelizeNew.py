...
parser.add_argument("--target-l2", type=float, default=1.0, help="Target L2 norm for the extinction fit.")
...
if ARG.verbose:
	...
	print("\nParsed Arguments:")
	for arg, value in vars(ARG).items():
		print(f"{arg}: {value}")
...
#Copy relevant arrays
	...
	def getMinimizer(self):
		def minimizer(x):
			...
			# Compute ff and ff_reduced with implicit first coefficient as 1
			ff = tf.constant(self.fitVec[0], dtype=self.dtype) + \
				 reduce(tf.add, (x[i] * tf.constant(self.fitVec[i + 1], dtype=self.dtype) for i in range(self.n - 1)))
			ff_reduced = tf.constant(self.fitVec_reduced[0], dtype=self.dtype) + \
						 reduce(tf.add, (x[i] * tf.constant(self.fitVec_reduced[i + 1], dtype=self.dtype) for i in range(self.n - 1)))
			# Normalize by sqrt(1 + x[0]^2 + ... + x[n-2]^2)
			norm_factor = tf.sqrt(1.0 + tf.reduce_sum(tf.square(x)))
			ff /= norm_factor
	...
	def getTVMinimizer(self):
		def tv_minimizer(x):
			x_norm = tf.sqrt(tf.reduce_sum(tf.square(x)+1.0))#Norm of the vector where the first implicit element is 1.0
			b_reduced_tf = tf.constant(self.b_reduced, dtype=self.dtype)
			ff_reduced =  tf.constant(self.fitVec_reduced[0], dtype=self.dtype)
			for k in range(1, self.n):
				ff_reduced += x[k-1] * tf.constant(self.fitVec_reduced[k], dtype=self.dtype)
			ff_reduced = tf.math.divide(ff_reduced, x_norm)
	...
	def reportMinimizerValue(self, x, x0=None):
		minimizer = self.getMinimizer()
		tv_value = minimizer(x).numpy()
		report = f"x: {x}"
		if x0 is not None:
			tv_value_x0 = minimizer(x0).numpy()
...
with tf.device('/cpu:0'):
	if ARG.first_fit is not None:
		...
		#x0[0] = 1.0
		AGM = AlphaGammaMinimizer(b0, basisVec, np.ones(basisVec.shape[0]), alpha, gamma, weight_l1=0, weight_tv=1, weight_neg=0, dtype=tf.float32)
		...
		if ARG.verbose:
			...
			print(AGM.reportMinimizerValue(x, x0), flush=True)

	#Compute properly scaled fit vector
	fitScale = np.ones_like(basisVec, dtype=np.float32)#Here I avoid using fitScale from previous step
	...
	for k in np.arange(frameCount):
		...
		x_full = np.concatenate([np.array([1.0], dtype=np.float32), x])
		x_full_scaled = x_full / np.linalg.norm(x_full) #Normalize to one
		x_full_scaled = x_full_scaled * ARG.target_l2
		ff_now = softplus(softplus(np.tensordot(x_full_scaled, basisVec, axes=[0,0])))
