...
parser.add_argument("--angles-mat",
                    type=str,
                    default=None,
                    help="Sequence of angles stored in mat file.")

ARG = parser.parse_args([
    inputFolder, "--output-folder", outputFolder, "--force", "--gpu", "--sirt",
    "--verbose", "--saveden", "--first-index", "450", "--last-index", "460"
])
...
vectors = generateEquispacedAstraParallel3d_vec(angles_count, det_width,
                                                det_height)
...
for i in range(row_count):
	...
	if ARG.verbose and i % 10 == 0:
		print("Just read %d-th/%d projectionData of %s" %
		      (i + 1, row_count, tifFiles[i]))
if ARG.neglog:
	projectionData = np.log(np.reciprocal(projectionData))

if ARG.verbose:
	print(
	    "Transformed projections of dimensions %dx%dx%d and dtype=%s with min=%f, max=%f, mean=%f."
	    % (projectionData.shape[0], projectionData.shape[1],
	       projectionData.shape[2], projectionData.dtype, projectionData.min(),
	       projectionData.max(), projectionData.mean()))
...
if ARG.verbose:
	print(
	    ...
	    % (det_width, det_height, len(angles)))
