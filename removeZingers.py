...
def process_frame(k, inputFile, outputFile, threashold, verbose=False):
	f = DEN.getFrame(inputFile, k)
	f_cor = rem.remove_zinger(f, threashold)
...
for k in range(dimz):
	pool.apply_async(process_frame, args=(k, ARG.inputDen, ARG.outputDen, ARG.threshold, ARG.verbose))
#	process_frame(k, ARG.inputDen, ARG.outputDen, ARG.threshold, ARG.verbose)
